...
import {
  Search, Plus, Edit, Trash2, Shield,
...
const SYSTEM_COLORS: Record<string, string> = {
  ...
  staff: "text-blue-700 bg-blue-50 border-blue-200",
  customer: "text-gray-700 bg-gray-50 border-gray-200",
};
const DEFAULT_COLOR = "text-purple-700 bg-purple-50 border-purple-200";
const AVATAR_COLORS: Record<string, string> = {
  admin: "bg-red-100 text-red-700",
  staff: "bg-blue-100 text-blue-700",
};
const DEFAULT_AVATAR = "bg-accent/10 text-accent";

function getRoleColor(name: string) { return SYSTEM_COLORS[name] || DEFAULT_COLOR; }
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/roles")
      .then((r) => r.json())
      .then((d) => {
        const list: RoleOption[] = (d.roles || []).map((r: Any) => ({
          value: r.name,
      ...
      .catch(() => {
        setRoles([
          { value: "admin", label: "Admin", desc: "Full access", color: SYSTEM_COLORS.admin },
          { value: "staff", label: "Staff", desc: "Manage content", color: SYSTEM_COLORS.staff },
          { value: "customer", label: "Customer", desc: "Storefront only", color: SYSTEM_COLORS.customer },
        ]);
      });
  }, []);
      ...
      <div className="flex flex-wrap gap-3">
        {roles.map((r) => (
          <div key={r.value} className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs ${r.color}`}>
            ...
            {r.desc && <span className="opacity-70">— {r.desc}</span>}
          </div>
        ))}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={formRole} onChange={(e) => setFormRole(e.target.value)} className="w-full bg-surface border-0 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-accent">
                {roles.map((r) => <option key={r.value} value={r.value}>{r.label}{r.desc ? ` — ${r.desc}` : ""}</option>)}
              </select>
            </div>
                  ...
                  <tr key={u.id} className="hover:bg-surface/50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${
                          AVATAR_COLORS[u.role] || DEFAULT_AVATAR
                        }`}>
                    ...
                    <td className="px-5 py-3.5">
                      <select
                        ...
                        className={`text-xs font-semibold rounded-full px-2 py-1 border cursor-pointer ${getRoleColor(u.role)}`}
                      >
                        {roles.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
                      </select>
                    </td>
